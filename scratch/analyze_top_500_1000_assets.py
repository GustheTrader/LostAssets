...
def find_top_assets():
    ...
    # Heap will store top 1000 largest values.
    top_heap = []
    limit = 1000
    ...
    # Slice to rank 501 to 1000 (0-indexed indices 500 to 1000)
    top_500_1000_assets = top_assets[500:1000]
    print(f"Selected {len(top_500_1000_assets)} assets in rank range 501-1000.")
    
    if not top_500_1000_assets:
        print("Error: No assets in the 501-1000 range.")
        return
        
    # Get the unique owner names of these 501-1000 assets
    top_owners = set(asset[3] for asset in top_500_1000_assets)
    print(f"Unique owner names in range 501-1000: {len(top_owners)}")
    ...
    for idx, (amount, file_idx, row_idx, owner, row) in enumerate(top_500_1000_assets):
    ...
    with open("scratch/top_500_1000_assets_report.json", "w", encoding="utf-8") as jf:
        json.dump(report_data, jf, indent=2)
        
    # Write the compiled detailed CSV of all portfolios
    csv_output_path = "scratch/california_top_500_1000_portfolio.csv"
...
def generate_markdown_report(report_data, csv_path):
    report_path = "scratch/california_top_500_1000_assets_report.md"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# California Ranks 501-1000 Unclaimed Assets & Bundled Portfolio Report\n\n")
        f.write("## Executive Summary\n\n")
        f.write(f"- **Total Combined Portfolio Value (Ranks 501-1000)**: ${total_unclaimed_value:,.2f}\n")
        ...
        f.write(f"- **Downloadable CSV Export**: [california_top_500_1000_portfolio.csv](file:///{os.path.abspath(csv_path).replace(os.sep, '/')})\n\n")
        
        f.write("## Ranks 501-1000 Owners & Value Summary\n\n")
        ...
        f.write("Below is the detail of all properties associated with each owner from Rank 501 to 600. The full detail for all 500 owners is exported in the CSV file.\n\n")
