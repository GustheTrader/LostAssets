...
const DEFAULT_STATE_RULES: Record<string, StateRule> = Object.fromEntries(
  Object.entries(STATE_DIRECTORY).map(([state, entry]) => [
    state,
    {
      state,
      name: entry.agency,
      website: entry.officialUrl,
      claimProcess: "Search the official state program or MissingMoney, then follow the state claim workflow. Verify current requirements before outreach or filing.",
      documentationRequired: ["Government-issued photo ID", "SSN/TIN proof if requested", "Proof tying owner to reported address or business", "Estate documents if claiming as heir or representative"],
      timeline: "Varies by claim type and state. Verify with the official program.",
      maxCommission: "Verify current state finder-fee law before outreach.",
    },
  ]),
) as Record<string, StateRule>;

const OVERRIDE_STATE_RULES: Record<string, StateRule> = {
  CA: {
    state: "CA",
  ...
  FL: {
    ...
    claimProcess: "Search online, start a claim, and submit required identity and ownership documentation through the official portal.",
    documentationRequired: ["Photo ID", "Proof of SSN/TIN", "Proof of reported address or business relationship"],
